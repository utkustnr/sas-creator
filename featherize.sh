...
simg2img "$srcFile" feather.img || cp "$srcFile" feather.img

rm -Rf tmp
mkdir -p d tmp
e2fsck -y -f feather.img
resize2fs feather.img 5000M
e2fsck -E unshare_blocks -y -f feather.img
mount -o loop,rw feather.img d
...
e2fsck -f -y feather.img || true
resize2fs -M feather.img
